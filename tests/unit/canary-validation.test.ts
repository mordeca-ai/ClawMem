  ...
  it("a fully-embedded vault with an unavailable preflight exits NONZERO and persists taint", async () => {
    ...
    const proc = Bun.spawnSync(["bun", "src/clawmem.ts", "embed"], {
      ...
      env: {
        ...
        // Fork hardening (9jyc0): on hosts where 127.0.0.1:9 silently DROPS instead of
        // refusing (observed on WSL2), an "unreachable" fetch hangs rather than erroring.
        // The fork's 1d1fn remote-fetch deadline bounds every probe, making this test
        // deterministic regardless of the host's RST behavior. Upstream (no default
        // bound) times this test out on such hosts.
        CLAWMEM_REMOTE_FETCH_TIMEOUT_MS: "2000",
      },
    });

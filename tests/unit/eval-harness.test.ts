...
describe("eval CLI (subprocess)", () => {
  ...
  const cliEnv = {
    ...
    // Fork hardening (9jyc0): hosts where a dead port silently DROPS instead of
    // refusing (observed on WSL2) hang each remote call; the fork's 1d1fn fetch
    // deadline bounds them so the subprocess finishes inside the test budget.
    CLAWMEM_REMOTE_FETCH_TIMEOUT_MS: "1500",
  };
